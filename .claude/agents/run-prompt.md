...
Executor options:
- **tdd**: Test-Driven Development flow (test-creator → coder-orchestrator → coder → standards → tester)
- **coder**: Direct implementation flow (coder-orchestrator → coder → standards → tester)
- **general-purpose**: Non-code tasks (research, documentation, analysis)
- **bdd**: BDD-driven TDD flow with Gherkin context (test-creator → coder-orchestrator → coder → standards → tester). Same as TDD but prompts include Gherkin scenarios that guide test creation.
...
3. Route based on executor type:
   ...
   - **Step 2**: Invoke Task tool with subagent_type="coder-orchestrator" with BOTH:
     * Original prompt content
     * Test files created by test-creator
     * Instruction: "Delegate this task to coder to make tests pass"
   - **Coder-orchestrator** extracts single task and invokes coder
   - **Coder agent** implements the task to pass the tests (TDD Green phase)
   - Wait for coder to complete
   - **Step 3**: SubagentStop hook on coder → invoke coding-standards-checker
   - Wait for standards check to complete
   - **Step 4**: SubagentStop hook on standards-checker → invoke tester
   - Wait for tester to complete
   - **Step 5**: SubagentStop hook on tester checks for remaining tasks
     * If tasks remain → loop back to coder-orchestrator
     * If all tasks done → TDD cycle complete
   - Full TDD cycle per task: Red (tests) → Green (implementation) → Quality gates → Loop
   </tdd_task_routing>

   <code_task_routing>
   If executor == "coder":
   - Invoke Task tool with subagent_type="coder-orchestrator" and prompt content
   - **Coder-orchestrator** extracts single task and invokes coder
   - **Coder agent** implements the task directly (no test-creator phase)
   - Wait for coder to complete
   - SubagentStop hook on coder → invoke coding-standards-checker
   - SubagentStop hook on standards-checker → invoke tester
   - SubagentStop hook on tester checks for remaining tasks
     * If tasks remain → loop back to coder-orchestrator
     * If all tasks done → implementation complete
   - Direct implementation workflow with per-task quality gates
   </code_task_routing>
...
3. **Execute all prompts in PARALLEL in a SINGLE MESSAGE**:
   ...
   - TDD/BDD tasks: Invoke Task tool with subagent_type="test-creator" (tests created first, then coder-orchestrator → coder invoked sequentially)
   - Code tasks: Invoke Task tool with subagent_type="coder-orchestrator" (orchestrator delegates to coder)
   - General tasks: Invoke Task tool with subagent_type="general-purpose"

   **IMPORTANT**: TDD tasks CANNOT be parallelized internally (test-creator must finish before coder-orchestrator → coder). However, multiple TDD tasks can start in parallel, each running their own test-creator → coder-orchestrator → coder sequence.
   ...
   **CRITICAL**: All initial tool invocations (Task tools) MUST be in a single message for true parallel execution.
   ...
   - Task tool with subagent_type="coder-orchestrator" for prompt 007 (direct code task)
   ...
   <important_note>
   - TDD tasks: test-creator → coder-orchestrator → coder → standards → tester (sequential within each task)
   - Code tasks: coder-orchestrator → coder → standards → tester (sequential within each task)
   ...
   - Each coder-orchestrator invocation triggers SubagentStop hooks for quality gates
   </important_note>
...
3. Route based on executor:
   - If "tdd" or "bdd": Invoke Task tool with subagent_type="test-creator" → wait for tests → invoke Task tool with subagent_type="coder-orchestrator" → wait for implementation → wait for quality gates (standards → tester via hooks)
   - If "coder": Invoke Task tool with subagent_type="coder-orchestrator" → wait for implementation → wait for quality gates (standards → tester via hooks)
...
- TDD flow is sequential within each task: test-creator → coder-orchestrator → coder → standards → tester
...
- Code tasks (default) → TDD workflow: test-creator → coder-orchestrator → coder → standards → tester
- Direct code tasks → coder workflow: coder-orchestrator → coder → standards → tester (skip test-creator)
...
- **TDD Flow**: test-creator creates tests → coder-orchestrator delegates → coder implements → standards-checker → tester → loop check
- **Direct Code Flow**: coder-orchestrator delegates → coder implements → standards-checker → tester → loop check
- **General Flow**: general-purpose agent executes → no quality gates
- **Per-Task Quality Gates**: Standards-checker and tester run AFTER EACH coder task, not batched at end
- SubagentStop hooks on coder trigger coding-standards-checker
- SubagentStop hooks on standards-checker trigger tester
- SubagentStop hooks on tester check for remaining tasks (loop back to coder-orchestrator if tasks remain)
...
---
```
(Full TDD: test-creator → coder-orchestrator → coder → standards → tester)
...
---
```
(BDD-driven TDD: Same as TDD but with Gherkin scenarios in `<gherkin>` tags guiding test creation. Used by gherkin-to-test agent for BDD pipeline. Flow: test-creator → coder-orchestrator → coder → standards → tester)
...
---
```
(Direct implementation: coder-orchestrator → coder → standards → tester, skip test-creator)
...
✅ **Lean Context**: coder-orchestrator ensures coder receives ONE task at a time, preventing context overload
</workflow_integration>
...
- Execution flow:
  ...
  2. coder-orchestrator receives tests and delegates single task to coder
  3. coder implements component to pass tests
  4. coding-standards-checker verifies code quality
...
- Routed to: coder-orchestrator → coder (direct implementation, skip test-creator) ✓
- Execution flow:
  1. coder-orchestrator receives task and delegates to coder
...
- Execution flow:
  ...
  2. coder-orchestrator receives tests and delegates task to coder
  3. coder implements authentication to pass all tests
  4. coding-standards-checker verifies code quality
  5. tester validates all scenarios pass

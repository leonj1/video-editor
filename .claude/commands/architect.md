...
> "Gherkin-to-test agent completed. Invoke run-prompt agent with: Task(subagent_type=\"run-prompt\", prompt=\"[numbers] --sequential\")"

### 6. **Run-Prompt Phase** (TDD Implementation)
When you see the gherkin-to-test completion signal, invoke `run-prompt` agent with the prompt numbers provided in the signal.

Example: If signal says "Task(subagent_type=\"run-prompt\", prompt=\"006 007 008 --sequential\")", execute:
```text
Task(subagent_type="run-prompt", prompt="006 007 008 --sequential")

...
{
  "continue": true,
  "systemMessage": "Gherkin-to-test agent completed. Created $PROMPT_COUNT BDD prompt file(s). Invoke run-prompt agent with: Task(subagent_type=\"run-prompt\", prompt=\"$BDD_PROMPTS --sequential\")"
}
EOF
else
  cat <<EOF
